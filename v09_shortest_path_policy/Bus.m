    ...
    methods
        ...
        function [obj, stations, students] = handleStart(obj, stations, students, G)
            if ~isempty(stations(obj.currentNode).queue)
                ...
                obj = obj.planNextMove(G, stations);
        ...
        function obj = planNextMove(obj, G, stations)
            % ----------------------------------
            % 1. Find stations with waiting students
            % ----------------------------------
            queueSizes = arrayfun(@(s) length(s.queue), stations);
            targets = find(queueSizes > 0);
        
            % If no demand anywhere → fallback to random
            if isempty(targets)
            ...
            % ----------------------------------
            % 2. Find closest target (shortest path)
            % ----------------------------------
            ...
            for t = targets
                [path, dist] = shortestpath(G, obj.currentNode, t);
        
                if ~isempty(path) && dist < bestDist
                    bestDist = dist;
                    bestTarget = t;
                    bestPath = path;
                end
            ...
            % ----------------------------------
            % 3. Use the shortest path
            % ----------------------------------
            if isempty(bestPath)
                % fallback safety
                obj.nextNode = obj.currentNode;
                obj.path = obj.currentNode;
                obj.pathIndex = 1;
                return;
            end
